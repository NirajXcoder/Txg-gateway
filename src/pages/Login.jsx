...
export default function Login({ onLoginSuccess }) {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || 'Failed to send OTP. Please start the bot first.');
      }
    } catch (err) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch('https://txg-gateway-2.onrender.com/api/auth/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ telegramId, password, otp })
      });
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#090d16] text-white px-4">
      <div className="max-w-md w-full bg-[#131b2e] border border-slate-800 rounded-2xl p-8 shadow-2xl">
        ...
        <p className="text-sm text-slate-400 text-center mb-6">Login to access your TXG Gateway Dashboard</p>
